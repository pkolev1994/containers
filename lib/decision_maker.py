...
class DecisionMaker():
	...
	def update_platform_status(self):
		...
		###orchastrator.json way
		###etcd way
		for host in self.etcd_manager.get_etcd_config()['swarm_servers']:
		###etcd way
			names_by_hosts[host] = {}
			...
			for container in docker_api.containers.list():
				...
				if app_name_search:
					...
					names_by_hosts[host][app_name].update( \
							{container.name: self.orchastrator_config['types_instances'][app_name][container.name]})
		self.etcd_manager.write("/orchastrator/platform_status", str(names_by_hosts))
	...
	def check_for_releasing_node(self):
		...
		curr_nodes_number = len(self.orchastrator_config['swarm_servers'])
		validation_flag = True
		for app in apps_count.keys():
			app_count = apps_count[app]
			app_per_node = '{}_per_node'.format(app)
			if curr_nodes_number*self.orchastrator_config[app_per_node] - app_count >= \
				self.orchastrator_config[app_per_node]:
				pass
			else:
		...
		if validation_flag:
			...
			for host in names_by_hosts.keys():
				if host == self.orchastrator_config['master']:
					continue
				curr_count = 0
	...
	def making_host_decision(self, application, decision, release_node=False):
		...
		if decision is 'up':
			...
			###orchastrator.json way
			###etcd way
			if average_app_number >= self.etcd_manager.get_etcd_config()[app_per_node]:
				if len(self.etcd_manager.get_etcd_config()['available_servers']) != 0:
					new_server = self.etcd_manager.get_etcd_config()['available_servers'][0]
					swarm_manager.join_server_swarm(host_ip = self.etcd_manager.get_etcd_config()['available_servers'][0])
			###etcd way
					return new_server
				else:
			...
			for host in app_by_hosts.keys():
				if app_by_hosts[host][application] < average_app_number and \
					app_by_hosts[host][application] < self.etcd_manager.get_etcd_config()[app_per_node]: #parse_config('orchastrator.json')[app_per_node]:
					return host
			for host in app_by_hosts.keys():
				return host
			...
			###etcd way
			logger.warning("Application => {} ||| min_apps on platform=> {}\tcurrent app_num {}". \
				format(application, self.etcd_manager.get_etcd_config()[min_app], application_number))
			###etcd way
			...
			###etcd way
			if application_number == self.etcd_manager.get_etcd_config()[min_app]:
			###etcd way
				return None
